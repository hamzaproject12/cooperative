...
const baseCooperatives: Cooperative[] = [];

type CsvCooperativeRow = {
  name: string;
  cercle: string;
  commune: string;
  douar: string;
...
function parseCsvData(source: string): CsvCooperativeRow[] {
...
function buildAddress(row: CsvCooperativeRow): string {
  const parts: string[] = [];
...
function mapAxisToCategory(axis: string, specialty: string): string {
  ...
  if (
    axisLower.includes('valorisation des produits locaux') ||
    specialtyLower.includes('patis') ||
    specialtyLower.includes('gateaux')
  ) {
    return 'Terroir';
  }
...
/** Chemin de base pour les images de coopératives (fichiers dans public/images/images/, servis sous /images/images/) */
const COOPERATIVE_IMAGES_BASE = '/images/images';
 ...
 */
function getLocalCoverImagePathFromAxisSpecialty(
  axis: string,
  specialty: string,
  imageKey: keyof typeof pexels
): string {
  const a = (axis ?? '').trim();
...
/** Images liées à l'axe et à la spécialité (Apiculture+Elevage des Reines → ruches, etc.) */
function pickImageSetKey(axis: string, specialty: string): keyof typeof pexels {
  const axisLower = axis.toLowerCase();
  const specialtyLower = specialty.toLowerCase();
...
function estimatePrice(axis: string, specialty: string): number {
  const axisLower = axis.toLowerCase();
  const specialtyLower = specialty.toLowerCase();
...
/** Produits par spécialité : images et textes liés à l’axe / spécialité (ex. Apiculture + Elevage des Reines). */
function getProductTemplatesForSpecialty(axis: string, specialty: string): ProductTemplate[] {
  ...
  if (a.includes('apiculture') && (s.includes('elevage des reines') || s.includes('reines'))) {
    return [
      { name: 'Reines d’abeilles', imageKey: 'ruches', price: 85, shortDesc: 'Reines sélectionnées pour l’élevage.', longDesc: 'Reines d’abeilles issues de l’élevage local, destinées au renouvellement des ruches et à l’apiculture.' },
      { name: 'Cage à reines', imageKey: 'ruches', price: 12, shortDesc: 'Cage de transport et introduction.', longDesc: 'Cage apicole pour le transport et l’introduction des reines en ruche.' },
      { name: 'Nourriture pour reines', imageKey: 'cire', price: 35, shortDesc: 'Gelée et candi pour l’élevage.', longDesc: 'Nourriture spéciale pour l’élevage des reines et le renforcement des colonies.' },
    ];
  }
  if (a.includes('apiculture') && (s.includes('caisses apicoles') || s.includes('caisses'))) {
    return [
      { name: 'Caisse apicole 10 cadres', imageKey: 'ruches', price: 180, shortDesc: 'Ruche en bois locale.', longDesc: 'Caisse apicole traditionnelle en bois, 10 cadres, pour l’apiculture de production.' },
      { name: 'Cadres à cire gaufrée', imageKey: 'cire', price: 25, shortDesc: 'Lot de 10 cadres.', longDesc: 'Cadres avec cire gaufrée pour équiper les ruches.' },
    ];
  }
  if (a.includes('apiculture') && (s.includes('cire') || s.includes('cire apicole'))) {
    return [
      { name: 'Cire d’abeille brute 500 g', imageKey: 'cire', price: 45, shortDesc: 'Cire naturelle des ruches.', longDesc: 'Cire d’abeille brute récoltée dans les ruches, pour bougies ou cosmétique.' },
      { name: 'Bougies en cire naturelle', imageKey: 'cire', price: 28, shortDesc: 'Bougies artisanales.', longDesc: 'Bougies fabriquées à la main à partir de cire d’abeille locale.' },
    ];
  }
  if (a.includes('apiculture') && (s.includes('candi'))) {
    return [
      { name: 'Candi apicole 1 kg', imageKey: 'cire', price: 42, shortDesc: 'Nourriture d’hiver pour abeilles.', longDesc: 'Candi apicole pour nourrir les colonies en période hivernale.' },
      { name: 'Miel toutes fleurs 500 g', imageKey: 'miel', price: 75, shortDesc: 'Miel de la coopérative.', longDesc: 'Miel récolté et mis en pot par les apiculteurs de la coopérative.' },
    ];
  }
  if (a.includes('apiculture') && (s.includes('miel'))) {
    return [
      { name: 'Miel toutes fleurs 500 g', imageKey: 'miel', price: 88, shortDesc: 'Miel récolté localement.', longDesc: 'Miel de la coopérative, récolté et conditionné sur place.' },
      { name: 'Miel 1 kg', imageKey: 'miel', price: 165, shortDesc: 'Format familial.', longDesc: 'Miel en pot d’1 kg, idéal pour les familles.' },
      { name: "Pollen d'abeille 250 g", imageKey: 'pollen', price: 55, shortDesc: 'Pollen séché.', longDesc: "Pollen d'abeille récolté à la ruche, riche en nutriments." },
    ];
  }
  if (a.includes('elevage') && s.includes('ovin')) {
    return [
      { name: 'Viande d’agneau (coupe au kg)', imageKey: 'elevage_ovin', price: 95, shortDesc: 'Viande locale.', longDesc: 'Viande d’agneau issue de l’élevage ovin traditionnel de la coopérative.' },
      { name: 'Laine brute lavée', imageKey: 'elevage_ovin', price: 35, shortDesc: 'Laine de mouton.', longDesc: 'Laine brute lavée, prête pour le tissage ou la vente.' },
    ];
  }
  if (a.includes('elevage') && s.includes('poulet')) {
    return [
      { name: 'Poulet fermier (pièce)', imageKey: 'volaille', price: 65, shortDesc: 'Élevage local.', longDesc: 'Poulet fermier élevé par la coopérative.' },
      { name: 'Œufs frais (lot 30)', imageKey: 'volaille', price: 28, shortDesc: 'Œufs du jour.', longDesc: 'Œufs frais issus de l’élevage poulet de la coopérative.' },
    ];
  }
  if (s.includes('couscous') || s.includes('pam+couscous')) {
    return [
      { name: 'Couscous traditionnel 1 kg', imageKey: 'couscous', price: 52, shortDesc: 'Semoule travaillée à la main.', longDesc: 'Couscous préparé selon les méthodes traditionnelles, à base de blé local.' },
      { name: 'Mélange PAM pour couscous 200 g', imageKey: 'herbes', price: 22, shortDesc: 'Plantes aromatiques et médicinales.', longDesc: 'Mélange de plantes aromatiques et médicinales pour parfumer le couscous.' },
    ];
  }
  if (s.includes('gateaux') || s.includes('patis')) {
    return [
      { name: 'Assortiment de gâteaux marocains', imageKey: 'pate', price: 48, shortDesc: 'Sachet 500 g.', longDesc: 'Gâteaux marocains traditionnels préparés par la coopérative.' },
      { name: 'Ghriba aux amandes', imageKey: 'pate', price: 38, shortDesc: 'Biscuit aux amandes.', longDesc: 'Ghriba maison aux amandes, recette traditionnelle.' },
    ];
  }
  if (s.includes('séchage') || s.includes('sechage')) {
    return [
      { name: 'Fruits secs mélangés 500 g', imageKey: 'sechage_fruits', price: 55, shortDesc: 'Abricot, figue, prune.', longDesc: 'Fruits séchés au soleil selon les méthodes traditionnelles.' },
      { name: 'Figues séchées 300 g', imageKey: 'sechage_fruits', price: 42, shortDesc: 'Figues nature.', longDesc: 'Figues séchées nature, sans additif.' },
    ];
  }
  if (s.includes('valorisation des produits locaux') || (a.includes('valorisation') && !s)) {
    return [
      { name: 'Panier découverte terroir', imageKey: 'herbes', price: 65, shortDesc: 'Sélection de produits locaux.', longDesc: 'Assortiment de produits valorisés par la coopérative.' },
      { name: 'Tisane aux plantes aromatiques 100 g', imageKey: 'herbes', price: 28, shortDesc: 'Mélange de plantes locales.', longDesc: 'Tisane à base de plantes aromatiques et médicinales du terroir.' },
    ];
  }
  if (s.includes('emballage') || s.includes('production d\'emballage')) {
    return [
      { name: 'Emballages artisanaux (lot)', imageKey: 'ceramique', price: 35, shortDesc: 'Emballages durables.', longDesc: 'Emballages produits par la coopérative pour les produits locaux.' },
    ];
  }
  return generic;
}

/** Rotation du tableau d'images pour que chaque produit ait une image principale différente (images[0] varie). */
function rotateImageSet<T>(arr: T[], offset: number): T[] {
  if (arr.length === 0) return arr;
  const n = offset % arr.length;
  return [...arr.slice(n), ...arr.slice(0, n)];
}

function createProductsForCsvCoop(coopId: string, row: CsvCooperativeRow): Product[] {
  ...
  const cooperativeCoverImage = getLocalCoverImagePathFromAxisSpecialty(
    row.developmentAxis,
    row.specialty,
    imageKey,
  );
  const productImages = [cooperativeCoverImage, cooperativeCoverImage, cooperativeCoverImage];
...
/** Génère le texte "Notre histoire" à partir des champs Excel (Cercle, Commune, Douar, Axe, Spécialité). */
function generateNotreHistoire(row: CsvCooperativeRow): string {
  ...
  return (
    `La coopérative ${row.name.trim()} a été créée dans le douar ${row.douar || '—'}, commune de ${row.commune || '—'}, dans le cercle de ${zone || '—'}. ` +
    `Spécialisée dans l’axe « ${axis} » et plus précisément dans « ${spec} », elle regroupe des producteurs et productrices locaux pour valoriser les savoir-faire et les ressources du territoire. ` +
    `Située à ${addr}, elle propose des produits issus de ${spec} et contribue au développement économique de la région. ` +
    `Aujourd’hui, ${row.name.trim()} met en avant la qualité et la traçabilité de ses productions.`
  );
}

function createCooperativesFromCsv(rows: CsvCooperativeRow[]): Cooperative[] {
...
export const cooperatives: Cooperative[] = [...baseCooperatives, ...csvCooperatives];

/**
 * 4 grandes catégories (familles de produits) pour la page d'accueil,
 * dégagées des colonnes Axe de développement et Spécialité.
 */
export type ProductTypeSlug = 'apiculture' | 'terroir' | 'patisserie' | 'elevage';
...
export function coopMatchesProductType(coop: Cooperative, typeSlug: ProductTypeSlug): boolean {
  ...
  switch (typeSlug) {
    ...
    case 'terroir':
      return (
        axis.includes('valorisation des produits locaux') ||
        spec.includes('pam') ||
        spec.includes('couscous') ||
        spec.includes('plantes aromatiques') ||
        spec.includes('séchage') ||
        spec.includes('sechage fruits') ||
        spec.includes('emballage') ||
        spec.includes("production d'emballage")
      );
    case 'patisserie':
      return (
        axis.includes('patisserie') ||
        axis.includes('patissereie') ||
        spec.includes('gateaux') ||
        spec.includes('patis')
      );
    case 'elevage':
      return (
        axis.includes('elevage ovin') ||
        spec.includes('elevage ovin') ||
        axis.includes('elevage poulet') ||
        spec.includes('elevage poulet') ||
        axis.includes('poulet') ||
        spec.includes('poulet')
      );
    default:
      return false;
  }
...
/** 4 grandes catégories pour le menu Types de produits (page d'accueil) – images locales (public/images/images/) */
...
/** Retourne les produits des coopératives dont l'axe/spécialité correspond au type demandé */
export function getProductsByProductType(typeSlug: string): ProductWithMeta[] {
  const slug = typeSlug as ProductTypeSlug;
  if (!productTypes.some((t) => t.slug === slug)) return [];
  return cooperatives
    .filter((coop) => coopMatchesProductType(coop, slug))
    .flatMap((coop) =>
      coop.products.map((p) => ({
        ...p,
...
/** Méta pour la page catégorie/type (titre, image, style) */
export function getProductTypeMeta(typeSlug: string): ProductTypeInfo | undefined {
